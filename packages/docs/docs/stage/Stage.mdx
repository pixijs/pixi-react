...
() =>
{
    ...
    return (
        <div style={{display: 'flex', flexDirection: 'column'}}>
            ...
            <button style={{margin: '20px auto 0 0'}} onClick={() => app.renderer.render(app.stage)}>Render</button>
        </div>
    );
}
```
...
```jsx
import { AppConsumer } from '@pixi/react-pixi';
...
```jsx
import { withPixiApp } from '@pixi/react-pixi';
