...
describe('hooks', () =>
{
    ...
    describe('useTick', () =>
    {
        const App = ({ children, cb }: { children: React.ReactNode; cb: (app: Application) => void }) =>
        {
            const app = useRef<Application>();
            const setApp = useCallback((_: Application) => (app.current = _), []);
        ...
        test('mount & unmount', () =>
        {
            ...
            const onMount = jest.fn((_app: Application) => (app = _app));
            ...
            const App = ({ withComp }: { withComp: boolean }) => (
                <Stage onMount={onMount}>
                    <Container>{withComp && <Comp />}</Container>
                </Stage>
            );
            ...
            act(() =>
            {
                render = renderer.create(<App withComp={false} />);
            });
            ...
            act(() =>
            {
                render.update(<App withComp={true} />);
                render.update(<App withComp={true} />);
                render.update(<App withComp={true} />);
            });
            ...
            act(() =>
            {
                render.update(<App withComp={false} />);
            });
        ...
        test('enable/disable', () =>
        {
            ...
            const testState = (enabled: boolean, calledTimes: number) =>
            {
                fn.mockClear();
        ...
        test('ticker fn.this should be ticker instance', () =>
        {
            ...
            const Counter = () =>
            {
                useTick(function tick(this: Ticker)
                {
                    fn(this);
                });
        ...
        test('clean up after unmount', () =>
        {
            ...
            const render = (withStage: boolean, withComp: boolean) => (
                <div>
                    {withStage && (
                        <App
                            cb={(_app) =>
            ...
            const { rerender, unmount } = reactTest.render(<></>);
            const updateRender = (stage: boolean, child: boolean) =>
            {
                if (add && remove)
                {
                    add.mockClear();
