...
describe('props', () =>
{
    ...
    describe('getTextureFromProps', () =>
    {
        ...
        {
            expect(() => getTextureFromProps('Test', root, { image: 123 })).toThrow('Test image prop is invalid');
        });

        test('invariant texture', () =>
        {
            expect(() => getTextureFromProps('Test', root, { texture: 'texture' })).toThrow(
        ...
        {
            expect(() => getTextureFromProps('Test', root, { video: 123 })).toThrow('Test video prop is invalid');
        });

        test('invariant source', () =>
        {
            expect(() => getTextureFromProps('Test', root, { source: null })).toThrow('Test source prop is invalid');
        });

        test('get texture from image url', () =>
        {
            const texture = getTextureFromProps('Test', root, { image: './image.png' });
        ...
        test('get texture from image html element', () =>
        {
            ...
            const texture = getTextureFromProps('Test', root, { image });
        ...
        {
            const texture = getTextureFromProps('Test', root, { video: './video.mp4' });
        ...
        test('get texture from video html element', () =>
        {
            ...
            const texture = getTextureFromProps('Test', root, { video });
        ...
        test('get no texture', () =>
        {
            expect(() => getTextureFromProps('Test', root, {})).toThrow('Test could not get texture from props');
        });

        test('get texture from texture', () =>
        {
            const texture = getTextureFromProps('Test', root, { texture: emptyTexture });
    ...
    describe('applyDefaultProps', () =>
    {
        ...
        {
            type TestContainer = Container & { foo: string };

            const changed = applyDefaultProps(instance, {}, { foo: 'bar', alpha: 0.5 });

            expect((instance as TestContainer).foo).toEqual('bar');
