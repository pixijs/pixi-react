...
describe('renderFromComponent', () => {
  ...
  test('invariant container', () => {
    expect(() => renderFromComponent('something', null)).toThrow(
      'Invalid argument `container`, expected instance of `PIXI.Container`'
    )
  })

  test('createContainer', () => {
    const res = renderFromComponent(element, app.stage, stageComponent, true, callback)
    expect(res).toEqual({ current: { child: { tag: 'TEXT' } } })
    expect(PixiFiber.createContainer).toHaveBeenCalledTimes(1)
    expect(PixiFiber.createContainer).toHaveBeenCalledWith(app.stage)
    expect(PixiFiber.injectIntoDevTools).toHaveBeenCalledTimes(1)
  ...
  test('updateContainer', () => {
    const res = renderFromComponent(element, app.stage, stageComponent, false, callback)
    expect(res).toBe(app.stage)
    expect(PixiFiber.createContainer).not.toHaveBeenCalled()
    expect(PixiFiber.updateContainer).toHaveBeenCalledTimes(1)
    expect(PixiFiber.updateContainer).toHaveBeenCalledWith(element, app.stage, stageComponent, callback)
    expect(PixiFiber.injectIntoDevTools).not.toHaveBeenCalled()
  })
})
