...
export default PixiComponent('ComponentName', {
  ...
  // prevent auto destroy instance on during clean up?
  // @default false
  preventAutoDestroy: false
})
```
...
By default components are destroyed at clean up (during reconciliation). To prevent this you can set `preventAutoDestroy` to `true`.
